...
struct RingView: View {
    ...
    private func ratio(for categoryIndex: Int) -> Double {
        guard
            ...
            let categoryAmount = Double(viewModel.totalAmountBy(category, includeCurrency: false)),
            let totalAmount = Double(viewModel.totalAmountOfAllPinnedItems(includeCurrency: false)),
            categoryAmount != 0,
            totalAmount != 0
        else { return 0.0 }
    ...
    private func offset(for categoryIndex: Int) -> Double {
        var offset = 0.0
        var previousIndex = categoryIndex
        while previousIndex > 0 {
            previousIndex -= 1
            offset += ratio(for: previousIndex)
        }
        return offset
    }
...
struct RingView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        VStack {
            ...
            RingView(viewModel: FilterableTransactionListViewModel(transactions: ModelData.sampleTransactions))
                .scaledToFit()
        }
        .padding()
