...
struct MockData {
    static let availableCategories: [FilterModel] = {
        var dictionary = [String: FilterModel]()
        ModelData.sampleTransactions.forEach({
            if let value = dictionary[$0.category.rawValue] {
                // do not add it again
            } else {
                dictionary[$0.category.rawValue] =
                FilterModel(
                    title: $0.category.rawValue,
        ...
        var all = [FilterModel(title: "all", color: .black, isSelected: true)]
